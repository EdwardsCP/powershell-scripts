...
#Create a PowerShell script that can be used to unblock SMB access on all shares on the file server, and re-enable the user's AD account after the incident is investigated and resolved
...
Write-Output "Script Automatically Generated to Enable AD Account and undo an SMB Block created by VaronisDisableUserDenySMB.ps1" | Out-File -FilePath "$UnblockScriptName" -Append
Write-Output "#>" | Out-File -FilePath "$UnblockScriptName" -Append
